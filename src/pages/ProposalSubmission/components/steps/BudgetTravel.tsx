...
import { BudgetMasterData, TravelItem, TravelType, YearwiseCost } from '../../types';

interface BudgetTravelProps {
    data: BudgetMasterData;
    updateData: <K extends keyof BudgetMasterData>(field: K, value: BudgetMasterData[K]) => void;
}

const TRAVEL_TYPES: TravelType[] = ['Inland travel', 'International travel'];

const createEmptyTravel = (): TravelItem => ({
    id: crypto.randomUUID(),
    travelType: 'Inland travel',
...
const BudgetTravel = ({ data, updateData }: BudgetTravelProps) => {
    const [formData, setFormData] = useState<TravelItem>(createEmptyTravel());
    const [searchTerm, setSearchTerm] = useState('');
    const [editingId, setEditingId] = useState<string | null>(null);

    const updateField = <K extends keyof TravelItem>(field: K, value: TravelItem[K]) => {
    ...
    const handleSave = () => {
        if (!formData.travelType) return;

        if (editingId) {
            updateData('travel', data.travel.map(item =>
                item.id === editingId ? { ...formData, id: editingId } : item
            ));
            setEditingId(null);
        } else {
            updateData('travel', [...data.travel, { ...formData, id: crypto.randomUUID() }]);
        }
    ...
    const handleDelete = (id: string) => {
        updateData('travel', data.travel.filter(item => item.id !== id));
    };
    ...
    const totalBudget = data.travel.reduce((sum, item) => sum + item.grossTotal, 0);
        ...
        <div className="space-y-8">
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-2">
                    ...
                    <Select value={formData.travelType} onValueChange={(v) => updateField('travelType', v as TravelType)}>
                ...
                <div className="space-y-2">
                    ...
                    <Textarea
                        placeholder="Provide detailed justification for travel..."
                ...
                <div className="space-y-2">
                    <Label className="text-sm font-medium">Year-wise Travel Cost Detail</Label>
            ...
            {data.travel.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                    <div className="flex items-center justify-between">
                        <h3 className="text-lg font-display font-semibold text-foreground">Travel Cost Detail</h3>
                    ...
                    <div className="border border-border rounded-xl overflow-hidden">
                        <Table>
                            <TableHeader>
                                <TableRow className="bg-primary text-white hover:bg-primary">
                                    <TableHead className="text-white font-semibold">Travel Type</TableHead>
                                    <TableHead className="text-white font-semibold">Year-1 (₹)</TableHead>
                                    <TableHead className="text-white font-semibold">Year-2 (₹)</TableHead>
                                    <TableHead className="text-white font-semibold">Year-3 (₹)</TableHead>
                                ...
                                <AnimatePresence>
                                    {data.travel.map((item) => (
                                        <motion.tr key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="hover:bg-muted/50">
                                            <TableCell className="font-medium">{item.travelType}</TableCell>
