...
import { BudgetMasterData, OverheadItem, YearwiseCost } from '../../types';

interface BudgetOverheadProps {
    data: BudgetMasterData;
    updateData: <K extends keyof BudgetMasterData>(field: K, value: BudgetMasterData[K]) => void;
}

const createEmptyOverhead = (): OverheadItem => ({
...
const BudgetOverhead = ({ data, updateData }: BudgetOverheadProps) => {
    const [formData, setFormData] = useState<OverheadItem>(createEmptyOverhead());
    ...
    const handleSave = () => {
        if (formData.grossTotal <= 0) return;

        if (editingId) {
            updateData('overhead', data.overhead.map(item =>
                item.id === editingId ? { ...formData, id: editingId } : item
            ));
            setEditingId(null);
        } else {
            updateData('overhead', [...data.overhead, { ...formData, id: crypto.randomUUID() }]);
        }
        setFormData(createEmptyOverhead());
    };
    ...
    const handleDelete = (id: string) => {
        updateData('overhead', data.overhead.filter(item => item.id !== id));
    };
    ...
    const totalBudget = data.overhead.reduce((sum, item) => sum + item.grossTotal, 0);
        ...
        <div className="space-y-8">
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-2">
                    <Label className="text-sm font-medium">Year-wise Overhead Detail</Label>
            ...
            {data.overhead.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                    <div className="flex items-center justify-between">
                        <h3 className="text-lg font-display font-semibold text-foreground">Overhead Detail Saved as Draft</h3>
