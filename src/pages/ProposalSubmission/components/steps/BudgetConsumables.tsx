...
import { BudgetMasterData, ConsumableItem, YearwiseCost } from '../../types';

interface BudgetConsumablesProps {
    data: BudgetMasterData;
    updateData: <K extends keyof BudgetMasterData>(field: K, value: BudgetMasterData[K]) => void;
}

const createEmptyConsumable = (): ConsumableItem => ({
...
const BudgetConsumables = ({ data, updateData }: BudgetConsumablesProps) => {
    const [formData, setFormData] = useState<ConsumableItem>(createEmptyConsumable());
    const [searchTerm, setSearchTerm] = useState('');
    const [editingId, setEditingId] = useState<string | null>(null);

    const updateField = <K extends keyof ConsumableItem>(field: K, value: ConsumableItem[K]) => {
    ...
    const handleSave = () => {
        ...
        if (editingId) {
            updateData('consumables', data.consumables.map(item =>
                item.id === editingId ? { ...formData, id: editingId } : item
            ));
            setEditingId(null);
        } else {
            updateData('consumables', [...data.consumables, { ...formData, id: crypto.randomUUID() }]);
        }
        setFormData(createEmptyConsumable());
    };
    ...
    const handleDelete = (id: string) => {
        updateData('consumables', data.consumables.filter(item => item.id !== id));
    };
    ...
    const totalBudget = data.consumables.reduce((sum, item) => sum + item.grossTotal, 0);
        ...
        <div className="space-y-8">
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-2">
                    ...
                    <Textarea
                        placeholder="Provide detailed justification for consumables..."
                ...
                <div className="space-y-2">
                    <Label className="text-sm font-medium">Year-wise Consumable Cost</Label>
            ...
            {data.consumables.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                    <div className="flex items-center justify-between">
                        <h3 className="text-lg font-display font-semibold text-foreground">Consumable Cost Detail Saved as Draft</h3>
...
export default BudgetConsumables;
